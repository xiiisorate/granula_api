// Package config handles Notification Service configuration.
package config
...
// Config holds all configuration for Notification Service.
type Config struct {
	AppEnv   string
	LogLevel string
	GRPC     GRPCConfig
...
func Load() *Config {
	return &Config{
		...
		GRPC: GRPCConfig{
			...
			Port: sharedconfig.GetEnvInt("GRPC_PORT", 50060),
		},
		DB: DatabaseConfig{
			...
			Name:     sharedconfig.GetEnv("DB_NAME", "notifications_db"),
