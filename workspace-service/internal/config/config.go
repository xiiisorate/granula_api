// Package config handles Workspace Service configuration.
package config
...
// Config holds all configuration for Workspace Service.
type Config struct {
	AppEnv   string
	LogLevel string
	GRPC     GRPCConfig
...
func Load() *Config {
	return &Config{
		...
		GRPC: GRPCConfig{
			...
			Port: sharedconfig.GetEnvInt("GRPC_PORT", 50053),
		},
		DB: DatabaseConfig{
			...
			Name:     sharedconfig.GetEnv("DB_NAME", "workspaces_db"),
